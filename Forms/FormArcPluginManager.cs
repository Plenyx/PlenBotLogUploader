...
public partial class FormArcPluginManager : Form
{
    ...
    internal FormArcPluginManager(FormMain mainLink)
    {
        ...
        switch (ApplicationSettings.Current.ArcUpdate.ChainLoad)
        {
            ...
            case ApplicationSettingsArcUpdateChainLoad.Nexus:
                ...
                break;
            case ApplicationSettingsArcUpdateChainLoad.None:
            default:
                break;
        }
        checkedListBoxArcDpsPlugins.ItemCheck += CheckedListBoxArcDpsPlugins_ItemCheck;
    ...
    private async Task CheckUpdatesAsync(bool applicationStart = false)
    {
        ...
        SetStatus($"{DateTime.Now.ToString(CultureInfo.CurrentCulture)}: Update check started.");
        ...
        {
            SetStatus($"{DateTime.Now.ToString(CultureInfo.CurrentCulture)}: Update check ended, no updates found.");
    ...
    private async Task UpdateArcAndPluginsAsync()
    {
        ...
        {
            SetStatus($"{DateTime.Now.ToString(CultureInfo.CurrentCulture)}: Updates for installed plugins found, updating...");
            ...
            SetStatus($"{DateTime.Now.ToString(CultureInfo.CurrentCulture)}: Updates successfully installed.");
            SetCheckNowButton(true);
            updateRunning = false;
        }
        else
        {
            SetStatus($"{DateTime.Now.ToString(CultureInfo.CurrentCulture)}: Updates for installed plugins found, waiting for GW2 to close...");
            if (checkBoxEnableNotifications.Checked && !updateManual)
            {
    ...
    private async void ButtonChangeGW2Location_Click(object sender, EventArgs e)
    {
        ...
        {
            Filter = "Guild Wars 2|Gw2-64.exe;Gw2.exe;Guild Wars 2.exe",
        };
    ...
    private async void CheckedListBoxArcDpsPlugins_ItemCheck(object sender, ItemCheckEventArgs e)
    {
        ...
        switch (e.NewValue)
        {
            ...
            case CheckState.Checked:
            {
                ...
                {
                    Type = item.Type,
                    RelativeLocation = $"\\{item.DefaultFileName}",
                };
                ArcDpsComponent.All.Add(component);
                await component.DownloadComponent(httpController);
                break;
