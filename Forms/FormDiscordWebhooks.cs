...
public partial class FormDiscordWebhooks : Form
{
        ...
        else // not WvW
        {
            ...
            if (reportJson.Players.Values.Count <= 10)
            {
                ...
                else
                {
                    ...
                    discordContentEmbedSquadAndPlayers.Add(squadEmbedField);
                    discordContentEmbedSquad.Add(squadEmbedField);
                    var numberOfRealTargets = reportJson.ExtraJson.Targets
                        .Count(x => !x.IsFake);
                    // damage summary
                    ...
                    var damageStats = reportJson.ExtraJson.Players
                        ...
                        {
                            Player = x,
                            DPS = numberOfRealTargets > 0 ? targetDps[x] : x.DpsAll[0].Dps,
                        })
    ...
    private async Task SendLogViaWebhooks(bool success, int bossId, bool isCm, bool isLegendaryCm, BossData bossData, List<LogPlayer> players, string jsonContentNone, string jsonContentSquad, string jsonContentPlayers, string jsonContentSquadAndPlayers)
    {
