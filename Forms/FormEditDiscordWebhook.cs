...
namespace PlenBotLogUploader
{
    public partial class FormEditDiscordWebhook : Form
    {
        ...
        public FormEditDiscordWebhook(FormDiscordWebhooks discordPingLink, int reservedId, bool addNew, DiscordWebhookData data)
        {
            ...
            {
                checkedListBoxBossesEnable.Items.Add(new BossesDisableHelperClass() { BossID = bosses[bossNumber].BossId, Text = $"{bosses[bossNumber].Type}: {bosses[bossNumber].Name} ({bosses[bossNumber].BossId})" }, data.IsBossEnabled(bosses[bossNumber].BossId));
            }
        }
